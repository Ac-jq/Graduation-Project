...
CREATE TABLE IF NOT EXISTS mental_scale_report (
    ...
    recommended_resource_ids VARCHAR(255) NULL COMMENT '推荐资源ID快照，逗号分隔',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
CREATE TABLE IF NOT EXISTS admin_ai_task (
    ...
    agent_status VARCHAR(32) NOT NULL DEFAULT 'CLARIFYING' COMMENT '智能体会话状态',
    summary_text VARCHAR(500) NULL COMMENT '任务摘要',
    failure_reason VARCHAR(500) NULL COMMENT '失败原因',
    pending_prompt VARCHAR(500) NULL COMMENT '待补充追问',
    conversation_log TEXT NULL COMMENT '多轮对话日志',
